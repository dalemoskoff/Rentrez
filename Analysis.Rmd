...
Data.frame<-as.data.frame(cbind(SequenceID,NumC,NumA,NumG,NumT)) # Create data frame table containing the number of each nucleotide for each of the three sequences.
...
#First, isolate just the sequence ID from the "Name" column of X, the original input data.

Z<-as.character(X$Name) # Specify "Name" column.
ID<-gsub(">","",Z) # Remove the ">" before the start of each sequence ID.
ID<-gsub(" .*","",ID) # Remove everything after the sequence ID.
